...
export default function VehiclesBulkImportPage() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await api.get<TemplateResponse>('/vehicles/bulk-import/template');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await api.post<ImportResult>('/vehicles/bulk-import', {
        fileName: file.name,
        base64,
      });
      setResult(res);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Bulk Import Vehicles</h1>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="rounded-lg border border-gray-200 bg-white p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
            ...
            <li>Fill in your vehicles (<code className="rounded bg-gray-100 px-1 text-xs">plate</code>, <code className="rounded bg-gray-100 px-1 text-xs">make</code> and <code className="rounded bg-gray-100 px-1 text-xs">model</code> are required)</li>
            <li>Link each row to an existing owner via <code className="rounded bg-gray-100 px-1 text-xs">customer_phone</code> or <code className="rounded bg-gray-100 px-1 text-xs">customer_name</code> — phone wins if both are given</li>
            <li>Upload — rows matched by plate are updated, others are created</li>
          </ol>
          <div className="mt-4 flex flex-wrap gap-3 text-xs text-gray-600">
            <span className="rounded-full bg-gray-100 px-2 py-1">Supported: .xlsx, .xls, .csv</span>
            <span className="rounded-full bg-gray-100 px-2 py-1">Customer must already exist in Customers</span>
            <span className="rounded-full bg-gray-100 px-2 py-1">fuel_type: petrol, diesel, electric, hybrid, lpg</span>
          </div>
          <div className="mt-4">
            <button
