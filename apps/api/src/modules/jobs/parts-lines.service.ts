...
export class PartsLinesService {
  ...
  async update(
    ...
    // Adjust stock if quantity changed on an issued line
    if (
      input.quantity !== undefined &&
      input.quantity !== Number(existing.quantity) &&
      existing.part_number
    ) {
      const qtyDiff = input.quantity - Number(existing.quantity); // positive = more stock to deduct
      const { data: part } = await this.supabase
        .getClient()
        .from('parts')
      ...
      if (part) {
        if (existing.stock_status === 'issued') {
          // Deducting more? Run the policy check on the delta.
          if (qtyDiff > 0) {
            const available = Number(part.stock_qty) - (Number(part.reserved_qty) || 0);
            await this.stockPolicyService.assertSufficientOrOverride(
              tenantId,
              'owner', // trust: line already exists; update doesn't have userRole plumbed through yet
              available,
              qtyDiff,
          ...
          if (whStock) {
            await this.supabase.getClient()
              .from('warehouse_stock')
              .update({ quantity: Number(whStock.quantity) - qtyDiff })
              .eq('id', whStock.id)
              .eq('tenant_id', tenantId);
          }
  ...
  async chargePlannedLine(tenantId: string, id: string, userId: string, userRole: string) {
    ...
    if (fetchError || !line) throw new NotFoundException('Parts line not found');
    ...
    // Reserve stock if linked to inventory
    if (partId) {
      const { data: part } = await client
        .from('parts')
      ...
      if (part) {
        ...
        await client
          .from('parts')
          .update({
            reserved_qty: (part.reserved_qty as number || 0) + qty,
            updated_by: userId,
          })
          .eq('id', partId)
          .eq('tenant_id', tenantId);
    ...
    // Update line status to charged + reserved
    const { data, error } = await client
      .from('parts_lines')
      .update({
        line_status: 'charged',
        stock_status: partId ? 'reserved' : null,
        reserved_at: partId ? new Date().toISOString() : null,
      })
      .eq('id', id)
      .eq('tenant_id', tenantId)
      .select()
