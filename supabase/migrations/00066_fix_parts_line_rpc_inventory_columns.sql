-- Fix: the RPC inserted `quantity_before` / `quantity_after` into
-- public.inventory_adjustments, but those columns don't exist. The
-- table only has quantity_change. CREATE OR REPLACE keeps the same
...
BEGIN
  ...
  IF v_has_part THEN
    ...
    -- inventory_adjustments schema (00006): id, tenant_id, part_id,
    -- quantity_change, reason, reference, adjusted_by, created_at.
    -- No quantity_before / quantity_after columns exist.
